...
class CurrentComponentsGridPING(CurrentComponents):
    ...
    def __init__(
            self, connectivity: Connectivity, params_synaptic: ParamsSynaptic, stimulus_currents
    ):
        super().__init__(connectivity)
        self._params_synaptic: ParamsSynaptic = params_synaptic
        self._stimulus_currents = stimulus_currents
        self._synaptic_currents = np.zeros(self.connectivity.params_ping.nr_neurons["total"])
    ...
    def get_synaptic_currents(self, dt, potentials):
        """
        Computes the new synaptic currents for postsynaptic neurons.
